...
<body class="bg-gray-100 flex min-h-screen">
    ...
    <div class="flex-1 flex flex-col">
        ...
        <main class="p-6 bg-gray-100 flex-1">
            <h3 class="text-xl font-bold text-gray-800 mb-4">ข้อมูลนักศึกษา</h3>
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) { 
                            echo "<tr>
                                    ...
                                    <td>{$row['email']}</td> 
                                    <td>
                                        <button class='bg-yellow-500 text-white px-4 py-2 rounded-lg' onclick='editStu({$row['id']})'>แก้ไข</button> 
                                    </td>
                                </tr>";
                        }
                    } else {
    ...
    <script>
        ...
        function logout() {
            if (confirm("คุณต้องการออกจากระบบหรือไม่?")) {
                window.location.href = "index.html"; // เปลี่ยนเป็นลิงก์หน้าเข้าสู่ระบบจริง
            }
        }
        function editStu(id) {
